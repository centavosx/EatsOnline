...
const Log = () => {
    ...
    const reverify = () =>{
        axios.patch(process.env.REACT_APP_APIURL+"reverify", encryptJSON({
            id: idn,
            name: name,
            email: email1
        })).then((response)=>{
    ...
    const verifyAccount = (e) =>{
        e.preventDefault();
        axios.patch(process.env.REACT_APP_APIURL+"verify", encryptJSON({
            id: idn,
            code: code,
        })).then((response)=>{
            response.data = decryptJSON(response.data.data)
    ...
    const loginAccount = (e) =>{
        e.preventDefault();
        axios.post(process.env.REACT_APP_APIURL+"login", encryptJSON({
            email: email1,
    ...
    return(
            ...
            <div className="user signinBx">
                <div className="imgBx"><img src="./assets/2 Login SignUp Image.png" alt=""/></div>
                ...
                <form style={{display:'grid'}} >
                        {/* <div className="row"> */}
                            ...
                            <input type="submit" name="" value="&#xf007; LOGIN / SIGN UP" onClick={()=>setLogin(true)} style={{maxWidth: '500px', width: '200px', height: '50px', fontFamily: 'Nunito', fontWeight: 'normal', fontStyle: 'normal', textDecoration: 'inherit'}}/>
                            {/* </div> */}
                                <h1 style={{textAlign: 'center'}}>OR</h1>
                            {/* <div className="col-lg-6 col-md-6 col-xs-12 col-sm-6"> */}
                            <div>
                                <input type="submit" onClick={(e)=>guest(e)} name="" value="&#xf2c0; LOGIN as GUEST" style={{maxWidth: '500px',  width: '200px', height: '50px', fontFamily: 'Nunito', fontWeight: 'normal', fontStyle: 'normal', textDecoration: 'inherit'}}/>
                                </div>
                            {/* </div> */}
                            <div>
                            <input type="submit" id="homeLog" onClick={()=>{history.push("/"); window.location.reload(false);}} className="homehome" value="&#xf015; Home" style={{fontFamily: 'Nunito',fontWeight: 'normal',fontSize: '17px', fontStyle: 'normal', textDecoration: 'inherit', textDecoration: 'underline', top: '100px', left:'45px', border: '2px solid #10505e'}}/>        
                            </div>
                    </form>
                :<form actionName="" onSubmit={(e)=>loginAccount(e)}>
                        <input type="submit" name="" value="BACK" onClick={(e)=>{e.preventDefault();setLogin(false);}}/>
                <h2>LOGIN</h2>
                    <label className="controllabel" style={{color: 'black'}}><span className="required">*</span>Email: </label>
                        <input type="email" name="" placeholder="Email Address" onChange={(e)=>setEmail1(e.target.value)}  required={true}/>
                            <label className="controllabel" style={{color: 'black'}}><span className="required">*</span>Password: </label>
                                ...
                                <span id="errororsuccess" style={{color:"red"}}>{error}</span><br/>
                                <input type="submit" name="" value="LOGIN" onClick={(e)=>setForget(false)}/>
                                    <p className="signup" style={{fontWeight: 'bold'}}>
                                    Don't have an account ? 
                                <a href={void(0)} onClick={toggleForm} style={{fontSize: '15px', textDecorationLine: 'underline' }}> Sign Up.</a>
                            </p>
                    ...
                    <div>  
                        <form onSubmit={(e)=>verifyAccount(e)}>
                            ...
                            <a style={{cursor:'pointer', color:'blue', textDecoration:'underline'}} onClick={()=>reverify()}>Resend verification code</a><br/>
                        ...
                        } 
                        {
                            ...
                            <input className="input-form" type="email"  name="email1" placeholder="Enter your email to send reset link." style={{color: 'black'}} required={true} />
                            <p ></p>
