...
const Signup = () => {
    ...
    const signup = (e) =>{
        ...
        if(check){
            axios.post(process.env.REACT_APP_APIURL+"register", encryptJSON({
                email: email,
    ...
    const validateInput = () =>{
        ...
        if(password.length>0 && confirmPass.length>0){
            if(!re.test(password) || !re2.test(password) || !re3.test(password) || !re4.test(password) || password.length<8){
                setPassError("Password should contain uppercase, lowercase and special characters and it should be equals or more than 8 characters");
                check = false;
            }else{
    ...
    <div className="user signupBx">
        <div className="formBx">
            <div>
                 <form action="" style={{marginTop: '40px'}} onSubmit={(e)=>signup(e)}>
                    <h2>Create an account</h2>
                    <label className="controllabel" style={{color: 'black'}}><span className="required">*</span>Name:</label>
                        {nameError.length>0?<><br/><span style={{color:'red', fontSize: '8px'}}>{nameError}</span></>:null}
                        ...
                        {emailError.length>0?<><br/><span style={{color:'red', fontSize: '8px'}}>{emailError}</span></>:null}
                        ...
                        {addressError.length>0?<><br/><span style={{color:'red', fontSize: '8px'}}>{addressError}</span></>:null}
                        ...
                        {numberError.length>0?<><br/><span style={{color:'red', fontSize: '8px'}}>{numberError}</span></>:null}
                        ...
                        {passError.length>0?<><br/><span style={{color:'red', fontSize: '8px'}}>{passError}</span></>:null}
                        ...
                        {confError.length>0?<><br/><span style={{color:'red', fontSize: '8px'}}>{confError}</span></>:null}
                            <input type="password" name="" placeholder="Confirm Password" onChange={e=>setConfirmPass(e.target.value)}/>
                                <span id="errororsuccess2" style={style}>{message}</span><br/>
                            <input type="submit" name="" value="SIGN UP" />
                                <p className="signup" style={{Top: '30px', fontWeight: 'bold'}}>
                                Already have an account ?
                                <a href={void(0)} onClick={toggleForm}  style={{fontSize: '15px', textDecorationLine: 'underline'}}> Log in.</a>
                                </p>
                        </form>
                    </div>
                </div>
